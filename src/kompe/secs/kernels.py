...
from kompe.cubed_sphere.spherical import enu_to_ecef

d2r = np.pi / 180


def clip_dot_product(x, delta=1e-7):
    """
    dot product clip:
...
def angular_distance(lat, lon, lat_secs, lon_secs, return_degrees=False):
    ...
    ecef_r_secs = np.vstack(
        (np.cos(la_s) * np.cos(lo_s), np.cos(la_s) * np.sin(lo_s), np.sin(la_s))
    ).T

    # the polar angles (N, M):
    theta = np.arccos(clip_dot_product(np.einsum("ij, kj -> ik", ecef_r_data, ecef_r_secs)))
...
def surface_current_matrices(
    ...
    constant=1.0 / (4 * np.pi),
    RI=EARTH_RADIUS_M + 110 * 1e3,
    singularity_limit=0,
):
    ...
    ecef_t = (
        ecef_r_secs[np.newaxis, :, :] - ecef_r_data[:, np.newaxis, :]
    )  # difference vector - not tangential yet
    ecef_t = (
        ecef_t
        - np.einsum("ijk,ik->ij", ecef_t, ecef_r_data)[:, :, np.newaxis]
        * ecef_r_data[:, np.newaxis, :]
    )  # subtract radial part of the vector to make it tangential
    ...
    R = np.hstack(
        (
            np.dstack((-np.sin(lo), np.cos(lo), np.zeros_like(la))),
    ...
    enu_t = np.einsum("lij, lkj->lki", R, ecef_t)[
        :, :, :-1
    ]  # remove last component (up), which should deviate from zero only by machine precicion
...
def magnetic_field_matrices(
    ...
    if current_type == "divergence_free":
        ...
        An_ = (
            MU0 * constant / (r * np.sin(theta))
        )  # common factor local northward (note sign difference wrt theta) direction
        Sn_ = np.zeros_like(theta)
        Sn_[below] = (sb - np.cos(theta[below])) / np.sqrt(
            1 + sb**2 - 2 * sb * np.cos(theta[below])
        ) + np.cos(theta[below])
        Sn_[above] = (1 - sa * np.cos(theta[above])) / np.sqrt(
            1 + sa**2 - 2 * sa * np.cos(theta[above])
        ) - 1
        Gn_ = An_ * Sn_
        ...
        if theta0 > 0:
            ...
            Ge_[theta < theta0] = (
                -MU0 * constant * alpha * np.tan(theta[theta < theta0] / 2) / rr[theta < theta0]
            )
        ...
        Ge = (
            Ge_ * enu_t[:, :, 1]
        )  # eastward component of enu_t is northward component of enu_e (unit vector in local east direction)
    ...
    if induction_nullification_radius is not None and current_type == "divergence_free":
        ...
        Ge_, Gn_, Gr_ = magnetic_field_matrices(
            lat, lon, r, lat_secs, lon_secs, current_type="divergence_free", RI=radius
        )
...
def current_wedge_magnetic_field_matrix(
    lat, lon, r, lat_I, lon_I, r_I, Ie, In, Ir, calculate_radial_leg=True
):
    ...
    Note
    ----
    The output of this function can be used in combination with magnetic_field_matrices for curl-free
    currents to correct for the effect of inclined field lines. See inclined_field_magnetic_field_matrices().
    ...
    ph_I, th_I, r_I = (
        np.deg2rad(current_params[1]),
        np.deg2rad(90 - current_params[0]),
        current_params[2],
    )
    s_ecef = r_I * np.vstack(
        (np.cos(ph_I) * np.sin(th_I), np.sin(ph_I) * np.sin(th_I), np.cos(th_I))
    )
    ...
    G_n = (
        -np.cos(th) * np.cos(ph) * G_ecef[0]
        - np.cos(th) * np.sin(ph) * G_ecef[1]
        + np.sin(th) * G_ecef[2]
    )
    G_r = (
        np.sin(th) * np.cos(ph) * G_ecef[0]
        + np.sin(th) * np.sin(ph) * G_ecef[1]
        + np.cos(th) * G_ecef[2]
    )

    if calculate_radial_leg:
        G_e_r, G_n_r, G_r_r = current_wedge_magnetic_field_matrix(
            lat, lon, r, lat_I, lon_I, r_I, Ie * 0, In * 0, Ir, calculate_radial_leg=False
        )
...
def inclined_field_magnetic_field_matrices(
    lat, lon, r, lat_secs, lon_secs, Be, Bn, Br, RI=EARTH_RADIUS_M + 110 * 1e3
):
    ...
    This function combines the standard SECS magnetic field, from magnetic_field_matrices with the magnetic field
    of a current wedge that consists of two semi-inifinite field lines, calculated with current_wedge_magnetic_field_matrix.

    Note
    ----
    This function does not (yet) support the singularity fix used in magnetic_field_matrices, so be careful when
    ...
    Ge_wedge, Gn_wedge, Gr_wedge = current_wedge_magnetic_field_matrix(
        lat,
        lon,
        r,
        lat_secs,
        lon_secs,
        np.full_like(lat_secs, RI),
        Be,
        Bn,
        Br,
        calculate_radial_leg=True,
    )
    Ge, Gn, Gr = magnetic_field_matrices(
        lat, lon, r, lat_secs, lon_secs, RI=RI, current_type="curl_free"
    )
