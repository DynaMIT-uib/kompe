    ...
    def add_km_grid(self, resolution, **kwargs):
        ...
        **kwargs : 2D line properties.
        ...
        for i in range(len(eta)):
            xi_pos.append(
                np.interp(
                    np.arange(0, self.grid.length / 2, resolution), np.cumsum(diff[i, :]), xi
                )
            )
        ...
        for i in range(len(eta)):
            xi_neg.append(
                np.interp(
                    np.arange(-resolution, -self.grid.length / 2, -resolution)[::-1],
                    np.cumsum(diff[i, :])[::-1],
                    xi[::-1],
                )
            )
        ...
        km_tick = np.concatenate(
            (
                np.arange(-resolution, -self.grid.length / 2, -resolution)[::-1],
                np.arange(0, self.grid.length / 2, resolution),
            )
        )
        ...
        for i in range(len(xi)):
            eta_pos.append(
                np.interp(
                    np.arange(0, self.grid.width / 2, resolution), np.cumsum(diff[i, :]), eta
                )
            )
        ...
        for i in range(len(xi)):
            eta_neg.append(
                np.interp(
                    np.arange(-resolution, -self.grid.width / 2, -resolution)[::-1],
                    np.cumsum(diff[i, :])[::-1],
                    eta[::-1],
                )
            )
        ...
        km_tick = np.concatenate(
            (
                np.arange(-resolution, -self.grid.width / 2, -resolution)[::-1],
                np.arange(0, self.grid.width / 2, resolution),
            )
        )
