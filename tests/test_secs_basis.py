...
def test_secs_scalar_synthesis_has_explicit_physical_mode(evaluation_grid):
    ...
    np.testing.assert_allclose(
        divergence_free.scalar_evaluation_matrix(evaluation_grid), expected_stream_function
    )
...
def test_surface_current_kernel_matches_canonical_components(
    ...
    canonical = secs_basis.surface_current_matrix(evaluation_grid, current_type=current_type)

    np.testing.assert_allclose(canonical[0], -north)
    np.testing.assert_allclose(canonical[1], east)
    operator = secs_basis.surface_current_operator(evaluation_grid, current_type=current_type)
...
def test_magnetic_field_uses_canonical_radial_theta_phi_order(
    ...
    canonical = secs_basis.magnetic_field_matrix(
...
def test_secs_rejects_invalid_current_type(secs_basis, evaluation_grid):
    with pytest.raises(ValueError, match="current_type"):
        secs_basis.surface_current_matrix(evaluation_grid, current_type="unknown")
    with pytest.raises(ValueError, match="current_type"):
        secs_basis.magnetic_field_matrix(evaluation_grid, 6371.2e3, current_type="unknown")
