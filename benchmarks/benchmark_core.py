...
def main():
    ...
    measure("Global CS Laplacian operator", cs_basis.get_surface_laplacian_operator, args.repeat)
    ...
    measure("SH scalar evaluation", lambda: sh_basis.evaluate_on_grid(target), args.repeat)
    ...
    measure(
        ...
        lambda: regional.operators.gradient_matrices(sparse=True),
        args.repeat,
    )

    poles = Grid(lat=regional.lat.reshape(-1)[::12], lon=regional.lon.reshape(-1)[::12])
    secs = SECSBasis(poles=poles, current_type="divergence_free")
    measure(
        "SECS current matrix",
        lambda: secs.get_surface_current_matrix(target, singularity_limit=1.0),
        args.repeat,
    )
    chunked_secs = secs.get_surface_current_operator(target, singularity_limit=1.0, chunk_size=100)
    measure(
